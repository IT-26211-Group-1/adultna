...
import type { BatchOperationResponse } from "@/hooks/queries/admin/useInterviewQuestionQueries";
import { logger } from "@/lib/logger";

type BatchInterviewQuestionActionsProps = {
  selectedQuestionIds: string[];
  onClearSelection: () => void;
  isArchiveView?: boolean;
};

export function BatchInterviewQuestionActions({
  selectedQuestionIds,
  onClearSelection,
  isArchiveView = false,
}: BatchInterviewQuestionActionsProps) {
  ...
  const handleBatchRestore = async () => {
    try {
      ...
      if (result.success) {
        ...
        if (result.results.failed.length > 0) {
          logger.error("Failed to restore questions:");
          result.results.failed.forEach((failure) => {
            logger.error(
              `  Question ID ${failure.questionId}: ${failure.reason}`,
            );
  ...
  const handleBatchPermanentDelete = async () => {
    try {
      ...
      if (result.success) {
        ...
        if (result.results.failed.length > 0) {
          logger.error("Failed to permanently delete questions:");
          result.results.failed.forEach((failure) => {
            logger.error(
              `  Question ID ${failure.questionId}: ${failure.reason}`,
            );
