...
const FileCard = memo(({ file, selectedRecentFile, onFileClick, onFileDoubleClick }: FileCardProps) => {
...
export const RecentFiles = memo(({ files, selectedRecentFile, onFileClick, onFileDoubleClick }: RecentFilesProps) => {
  return (
    <div className="mb-8">
      <div className="mb-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {files.slice(0, 4).map((file) => (
          <FileCard key={file.id} file={file} selectedRecentFile={selectedRecentFile} onFileClick={onFileClick} onFileDoubleClick={onFileDoubleClick} />
        ))}
      </div>
    </div>
  );
