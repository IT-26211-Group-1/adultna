...
export function FeatureNav() {
  ...
  return (
    <section className="w-full min-h-[50px] top-10 py-8 md:py-16 bg-white relative flex flex-col gap-2 px-2 md:px-22 max-w-6xl items-center justify-center">
      <nav
        className="w-full md:w-5xl py-4 bg-white shadow-md flex justify-center border-3 border-adult-green rounded-[30px]"
        id="feature-nav"
      >
        <ul className="flex flex-wrap justify-center gap-2 md:gap-8 font-inter w-full">
          {navItems.map((item) => (
            <button
              key={item.label}
              className="relative px-2 py-2 md:px-4 md:py-2 rounded-lg hover:text-ultra-violet cursor-pointer group w-full sm:w-auto text-sm md:text-base"
              onClick={() => handleScroll(item.target)}
            >
              {item.label}
              <span className="absolute left-0 bottom-0 w-full h-0.5 bg-ultra-violet scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left" />
            </button>
          ))}
        </ul>
      </nav>
